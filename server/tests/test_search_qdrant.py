...
def embed_mock() -> AsyncMock:
    return AsyncMock(return_value=FAKE_VEC)
...
async def test_wildcard_returns_all_notes(index: QdrantSearchIndex, tmp_path: Path) -> None:
    ...
    with patch.object(index, "_embed", AsyncMock(return_value=FAKE_VEC)):
...
async def test_sync_re_embeds_updated_note(index: QdrantSearchIndex, tmp_path: Path) -> None:
    ...
    with patch.object(index, "_embed", counting_embed):
        await index.search("*")

    initial_count = len(embed_calls)
    ...
    assert len(embed_calls) > initial_count
